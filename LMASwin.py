...
class DCFAM(nn.Module):
    def __init__(self, encoder_channels=(96, 192, 384, 768), atrous_rates=(6, 12), num_classes=6, dropout=0.05, bias=False):
        ...
        self.pla = PositionLinearAttention(in_places=encoder_channels[2])
        ...
        self.up43 = nn.Sequential(SeparableConvBNReLU(encoder_channels[-1], encoder_channels[-2], dilation=rate_1),
                                 nn.UpsamplingNearest2d(scale_factor=2)
                                 )
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7, x8):
        ...
        # print('upsample5:',upsample5.shape)#[4, 384, 64, 64]
        # print('concat5:', concat5.shape)

        # print('self.conv_4(x4):', self.conv_4(x4).shape)
        # print('x7:', x7.shape)
        # print('self.down34(x7):', self.down34(x7).shape)
        # out4 = self.conv_4(x4) + self.down24(self.down34(x6))#[4, 768, 32, 32]
        ...
        # print('x8:', x8.shape)
        # print('self.up43(x8):', self.up43(x8).shape)
        out3 = self.conv_3(x3) + x5 #[4, 384, 64, 64]
        # out3 = self.maxpool(self.maxpool(concat2))
        out2 = self.conv_2(x2) + self.lf4(self.maxpool(x8)) #[4, 192, 128, 128]
        # out2 = self.maxpool(self.maxpool(concat1))

        out1 = self.conv_1(x1) + self.lf3(self.maxpool(x7)) #[4, 96, 256, 256]
...
class Decoder(nn.Module):
    ...
    def forward(self,x1, x2, x3, x4, x5, x6, x7, x8):
        out1, out2, out3, out4 = self.dcfam(x1, x2, x3, x4, x5, x6, x7, x8)
...
class LMASwin(nn.Module):
    def __init__(self,
        ...
        self.supencoder = SE(encoder_channels=(96, 192, 384, 768), atrous_rates=(6, 12), num_classes=6, dropout=0.05, bias=False)
...
#         self.conv2 = nn.Conv2d(96, 192, kernel_size=3, stride=1, padding=1)
#         self.conv3 = nn.Conv2d(192, 384, kernel_size=3, stride=1, padding=1)
#         self.conv4 = nn.Conv2d(384, 768, kernel_size=3, stride=1, padding=1)
#         self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
#         self.fc = nn.Linear(768*4*32, num_classes)
#
#     def forward(self, x):
...
#         super().__init__()
#         self.swin = DCSwin(encoder_channels=(96, 192, 384, 768),
#                    num_classes=num_classes,
#                    embed_dim=96,
#                    depths=(2, 2, 18, 2),
#                    num_heads=(3, 6, 12, 24),
#                    frozen_stages=2)
#         self.cnn = CNN(in_channels=3, num_classes=num_classes)
...
def dcswin_base(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_base.pth'):
    ...
    model = LMASwin(encoder_channels=(128, 256, 512, 1024),
                   num_classes=num_classes,
                   embed_dim=128,
                   depths=(2, 2, 18, 2),
                   num_heads=(4, 8, 16, 32),
...
def bmnet_small(pretrained=True, num_classes=6, weight_path='pretrain_weights/stseg_small.pth'):
    model = LMASwin(encoder_channels=(96, 192, 384, 768),
                   num_classes=num_classes,
                   embed_dim=96,
                   depths=(2, 2, 18, 2),
