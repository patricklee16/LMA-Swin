...
from geoseg.models.LMASwin import lmaswin_small
...
weights_name = "bmnet-small-1024-ms-512crop-e70"
weights_path = "model_weights/vaihingen/{}".format(weights_name)
test_weights_name = "bmnet-small-1024-ms-512crop-e70"
...
#  define the network
net = lmaswin_small(num_classes=num_classes)
# net = PatchUpModel(net,num_classes=num_classes, block_size=7, gamma=.9, patchup_type='soft')
# net = PatchUpModel(net,num_classes=num_classes, block_size=7, gamma=.9, patchup_type='hard')

# define the loss
loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
                 DiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
# loss = OHEM_CELoss(ignore_index=ignore_index)
use_aux_loss = False
